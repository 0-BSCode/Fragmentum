 ...
 */
function positionButton(selectionRect) {
  if (!floatingButton) return;
 ...
 */
async function handleFragmentGeneration() {
  const selection = window.getSelection();
  const text = selection.toString().trim();
 ...
 */
function generateTextFragment(selection) {
 ...
 */
function extractContext(range, prefixWords, suffixWords) {
 ...
 */
function buildFragmentURL(parts) {
